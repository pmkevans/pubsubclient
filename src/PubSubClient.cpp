...
boolean PubSubClient::publish(const char* topic, const uint8_t* payload, unsigned int plength, boolean retained) {
  ...
  if (MQTT_MAX_PACKET_SIZE < MQTT_MAX_HEADER_SIZE + 2 + topicLength + plength) {
    ::sprintf(this->_statusMessage, "PubSubClient::publish: Payload was too long to send.  Max length %d, required length %lu", MQTT_MAX_PACKET_SIZE, MQTT_MAX_HEADER_SIZE + 2 + topicLength + plength);
    return false; 
  }
...
size_t PubSubClient::write(const uint8_t* writeBuffer, size_t size) {
  this->lastOutActivity = ::millis();
  ::sprintf(this->_statusMessage, "Wrote %lu, length %lu", writeBuffer, size);
...
boolean PubSubClient::subscribe(const char* topic, uint8_t qos) {
  ...
  if (MQTT_MAX_PACKET_SIZE < 9 + ::strlen(topic)) {
    ::sprintf(this->_statusMessage, "PubSubClient::subscribe: topic length %lu is greater than %d and not supported", ::strlen(topic), MQTT_MAX_PACKET_SIZE - 9);
...
boolean PubSubClient::unsubscribe(const char* topic) {
  if (MQTT_MAX_PACKET_SIZE < 9 + ::strlen(topic)) {
    ::sprintf(this->_statusMessage, "PubSubClient::unsubscribe: topic length %lu is greater than %d and not supported", ::strlen(topic), MQTT_MAX_PACKET_SIZE - 9);
    return false;
  }
